...
from run_gov_report import build_vanilla_prompt, get_context, build_worker_prompt, build_manager_prompt
...
def main():
    ...
    print("Loading GovReport...")
    dataset = load_dataset("THUDM/LongBench", "gov_report", split="test", trust_remote_code=True)
    if args.mode == "test":
    ...
    for model_id in MODELS:
        print(f"\nEvaluating Model: {model_id} on GovReport ({args.mode} mode)")
        ...
        try:
            ...
            print("\n--- 3. RAG Pipeline ---")
            run_rag(model, tokenizer, dataset, "GovReport", build_vanilla_prompt, compute_rouge, model_id)
